document.addEventListener('DOMContentLoaded', () => {
    ...
    const startTopicButton = document.getElementById('start-topic-button');
    const topicStatusDisplay = document.getElementById('topic-status');
    ...
    let studyTopic = "";
    // Valores usados únicamente por las funciones de exportación heredadas.
    let pdfTextContent = "";
    let currentPdfName = "Conocimiento incorporado";
    let currentPdfTitle = "";
    let pdf2TextContent = "";
    let currentPdf2Name = "";
...
Debes utilizar tu conocimiento incorporado para explicar el tema indicado por el estudiante. No dependes de documentos externos ni debes solicitar o mencionar un PDF.
...
- ¿Qué te pide el problema?
    - ¿Qué concepto del tema podría aplicarse?
...
## Si el estudiante solo quiere practicar
    ...
    Genera ejercicios similares relacionados con el tema de estudio.
...
Sé amable, paciente y motivador. Evita respuestas largas cuando no sean necesarias. Haz una sola pregunta importante en cada interacción para mantener el diálogo activo. No reveles todos los pasos de una vez. Espera la respuesta del estudiante antes de continuar.
    ...
    // ----- Event Listeners -----
    startTopicButton.addEventListener('click', startTopic);
    ...
    function resetChat() {
        ...
        topicStatusDisplay.textContent = "";
        chatHistory = [];
        chatDisplay.innerHTML = '<div class="system-message">Indica un tema de estudio para comenzar.</div>';
        questionInput.value = "";
        ...
        studyTopicInput.focus();
    }

    function startTopic() {
        const topic = studyTopicInput.value.trim();
        if (!topic) {
            topicStatusDisplay.textContent = "Escribe un tema para comenzar.";
            topicStatusDisplay.style.color = "var(--danger-color)";
            return;
        }
        studyTopic = topic;
        currentPdfName = topic;
        currentPdfTitle = topic;
        topicStatusDisplay.textContent = `Tema seleccionado: ${studyTopic}`;
        topicStatusDisplay.style.color = "var(--success-color)";
        chatHistory = [];
        chatDisplay.innerHTML = '';
        addMessageToChat(`Listo para ayudarte con «${studyTopic}» usando mi conocimiento general. ¡Puedes empezar!`, "system");
    ...
    studyTopicInput.addEventListener('keypress', event => {
        if (event.key === 'Enter') startTopic();
    });

    /* Legacy PDF upload handlers are kept below only for old downloaded clones. */
    ...
    async function handleSendQuestion() {
        ...
        if (!question || !studyTopic || isProcessing) {
            if (!studyTopic) addMessageToChat("Por favor, indica un tema de estudio primero.", "system error");
            return;
        }
        ...
        const systemPrompt = 'Eres un tutor educativo para el tema «' + studyTopic + '» y SOLO debes ayudar con preguntas relacionadas con ese tema usando tu conocimiento incorporado. No dependes de documentos externos ni debes solicitar un PDF. No respondas temas claramente ajenos. Si la pregunta NO tiene relación con el tema, responde únicamente una línea que empiece exactamente con [FUERA_DE_ALCANCE] seguida de una frase breve en español. Para preguntas válidas, fundamenta la respuesta en conocimientos académicos confiables y actúa siempre como tutor educativo.\n\n' +
             SYS_TUTOR +
             '\n\n(Instrucción: Para expresiones matemáticas, utiliza SÓLO el formato $$...$$ para bloque y $...$ para texto en línea.)\n\n' +
             'El tema de estudio seleccionado por el estudiante es: «' + studyTopic + '». Usa tu conocimiento incorporado sobre este tema como base de conocimiento.';
    ...
    function getSessionHTML() {
        ...
        return `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>Sesión de Chat</title>
            ...
            <h2>Sesión de Chat del tutor socrático</h2><p>Tema: ${studyTopic}</p><hr>
    ...
    function downloadSessionPdf() {
        ...
        element.innerHTML = `<div style="font-family:Arial,sans-serif;padding:20px;">
            <h2>Sesión de Chat sobre ${studyTopic}</h2><hr style="margin-bottom:20px;">${chatDisplay.innerHTML}</div>`;
    ...
    function buildFallbackSuggestedQuestions(pdfName, subtitle) {
        const t = String(subtitle || pdfName || 'el tema de estudio').replace(/</g, '');
        return [
            `¿Qué es ${t} y cómo se puede reconocer?`,
            `¿Cuáles son las características que distinguen a ${t}?`,
            `¿Cómo se clasifica ${t} y qué ejemplos representativos existen?`,
            `¿Dónde se encuentra ${t} y qué condiciones necesita?`,
            `¿Cómo funciona o se comporta ${t} en su entorno?`,
            `¿Qué funciones o usos cumple ${t}?`,
            `¿Qué factores favorecen o dificultan ${t}?`,
            `¿Cómo se relaciona ${t} con otros elementos o fenómenos?`,
            `¿Qué diferencias hay entre los principales tipos de ${t}?`,
            `¿Qué errores comunes se deben evitar al estudiar ${t}?`,
            `¿Qué ejemplo práctico ayuda a comprender mejor ${t}?`,
            `¿Qué problema o desafío actual está relacionado con ${t}?`
        ];
    }

    async function fetchSuggestedQuestionsForTopic(model, topic) {
        const prompt =
            `Eres un diseñador de preguntas educativas. El tema de estudio ingresado por el usuario es: "${topic}".\n\n` +
            'Genera exactamente 12 preguntas frecuentes, naturales y útiles para iniciar un diálogo con un tutor socrático sobre ese tema.\n' +
            'Adapta las preguntas al tipo de tema: si es un ser vivo, incluye características, clasificación, hábitat, comportamiento, alimentación, reproducción, relaciones ecológicas y conservación; si es una ciencia, incluye conceptos, mecanismos, variables, aplicaciones y ejemplos; si es historia, incluye contexto, causas, actores, hechos y consecuencias; si es una disciplina o habilidad, incluye fundamentos, procedimiento, ejemplos, errores y aplicaciones.\n' +
            'No uses preguntas vacías o metaacadémicas como "¿cuál es la idea principal de [tema]?", "¿qué debo saber sobre [tema]?" o "¿cómo puedo aprender [tema]?". No repitas mecánicamente el nombre completo del tema en todas las preguntas. Cada pregunta debe tener sentido por sí sola, ser concreta y poder responderse con conocimiento académico.\n\n' +
            'Responde ÚNICAMENTE con un array JSON válido de 12 cadenas, sin markdown ni texto adicional.';
        const res = await fetch(`${API_BASE}/text`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ prompt, model: model || TEXT_MODELS[0], json: true })
        });
        ...
        try {
            const parsed = JSON.parse(raw);
            if (parsed && typeof parsed === 'object') content = String(parsed.text ?? parsed.output ?? parsed.response ?? raw).trim();
        } catch (_) { /* respuesta en texto plano */ }
        const questions = parseSuggestedQuestionsFromApi(content);
        if (!questions || questions.length < 10) throw new Error('parse');
    ...
    async function downloadChatbot() {
        ...
        try {
            const gen = await fetchSuggestedQuestionsForTopic(selectedModel, userSubtitle);
            if (gen && gen.length >= 10) suggestedQuestions = gen;
        } catch (_) { /* mantener preguntas de respaldo */ }
...
<head>
    ...
    <title>Tutor socrático sobre ${userSubtitle}</title>
    <script>
        window.MathJax = {
...
<div class="container">
    <header>
        <h1>Tutor socrático sobre ${userSubtitle}</h1>
        <p>${userSubtitle}</p>
    </header>
    <div class="toolbar">
    ...
    <div class="pdf2-section">
        <label class="title">Tema de estudio</label>
        <div>El tutor utiliza su conocimiento incorporado sobre: <strong>${userSubtitle}</strong></div>
    </div>
...
const systemInstructions = 'Eres un tutor educativo que SOLO debe ayudar con preguntas relacionadas con el tema «' + CHATBOT_THEME + '» usando tu conocimiento incorporado. No dependes de documentos externos ni debes solicitar un PDF. ' +
            'No respondas temas claramente ajenos (vida personal sin relación, otros materiales, programación arbitraria sin vinculación al tema, etc.). ' +
            'Si la pregunta NO tiene relación con el tema, responde ÚNICAMENTE una línea que empiece exactamente con [FUERA_DE_ALCANCE] seguida de una frase breve en español. ' +
            'Para preguntas válidas, fundamenta la respuesta en conocimientos académicos confiables y actúa siempre como tutor educativo.' +
            '\\n\\n' + SYS_TUTOR_EMBEDDED +
            '\\n\\n(Instrucción: Para expresiones matemáticas, utiliza SÓLO el formato $$...$$ para bloque y $...$ para texto en línea.)\\n\\n' +
            'El tema de estudio seleccionado es «' + CHATBOT_THEME + '». Usa tu conocimiento incorporado sobre este tema como base de conocimiento.';
        let conversationText = '';
    ...
    function resetEmbeddedChat() {
        ...
        chatDisplay.innerHTML = '';
        questionInput.value = '';
        addMsg('Listo para ayudarte con el tema «' + CHATBOT_THEME + '» usando mi conocimiento incorporado. ¡Puedes empezar!', 'system');
        questionInput.focus();
    }
